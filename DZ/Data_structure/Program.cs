...
namespace Data_structure
{
    internal class Program
    {
        ...
        static List<ShoppingProject> GenerateShoppingProjects(int count)
        {
                                            ...
                                            // для 4 показателей
            {
                ...
                decimal budget = random.Next(1000, 20001);
                projects.Add(new ShoppingProject(name, itemCount, shoppingtime, budget)); // Добавление проекта
                                                                                          // в пустой список
            }
            return projects; // Возврат результата
        }
            static List<ShoppingProject> FindTopProjects(List<ShoppingProject> projects, // Объявление метода
                Func<ShoppingProject, IComparable> selector, int topCount)
            {
                return projects
            ...
            static void PrintResults(List<ShoppingProject> projects) // Объявление метода
            {
